...
router.post("/functions/dungeonAction", async (req: Request, res: Response) => {
  try {
    ...
    if (action === "attack" || action === "skill") {
      ...
      if (usedSkillId) {
        // Create a pseudo "boss" object for effects targeting the boss
...
router.post("/functions/towerAction", async (req: Request, res: Response) => {
  try {
    ...
    if (action === "attack" || action === "skill") {
      ...
      // Enemy slow debuff: slowed enemies take 50% more damage
      ...
      for (let ei = 0; ei < enemies.length; ei++) {
        ...
        if (evaded) {
          ...
          if (actualDmg > 0 && (me.shield_hp || 0) > 0) {
            ...
            tShieldText = shieldBroken ? " (shield broken!)" : afterShield === 0 ? " (absorbed by shield)" : "";
            actualDmg = afterShield;
          }
          me.hp = Math.max(0, me.hp - actualDmg);
          d.combat_log.push({ type: "boss_attack", text: `${e.name} hits you for ${actualDmg}${blocked ? " (BLOCKED!)" : ""}${tShieldText}` });
        }
