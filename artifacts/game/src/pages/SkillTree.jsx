  ...
  return (
    <svg className="absolute inset-0 w-full h-full pointer-events-none" style={{ zIndex: 0 }}>
      {lines.map(line => {
        ...
        const width = line.bothLearned ? 2 : 1.5;
        const glow = line.bothLearned ? line.elemColor : "none";

        // Route: drop 10px from parent, go horizontal, then drop to child
        // This keeps the horizontal segment in the gap between tier rows
        ...
        return (
          <g key={line.id}>
            {line.bothLearned && (
              <path d={path} fill="none" stroke={glow} strokeWidth={5} strokeOpacity={0.12} strokeLinecap="round" strokeLinejoin="round" />
            )}
