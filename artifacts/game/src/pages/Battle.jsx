...
export default function Battle({ character, onCharacterUpdate }) {
  ...
  const doPlayerAttack = useCallback(async (skill = null) => {
    ...
    setTimeout(() => setShowAttackVisual(false), 1600);
  ...
  return (
    ...
    <div className="p-4 md:p-6 max-w-5xl mx-auto space-y-4 relative z-[1]">
      ...
      <div ref={arenaRef} className="grid md:grid-cols-[1fr_auto_1fr] gap-4 items-start relative">
        ...
        >
          {playerNumNode}
          {/* Buff zone — absolutely positioned starting at ~55% from the left of
              the card. NOT next to the name (which would overlap the name/HP area)
              and NOT pinned to the far-right corner. Out of the normal flex flow
              so HP/MP/EXP never shift when buffs appear or stack up. */}
          {activePlayerBuffs.length > 0 && (
            <div className="absolute top-4 left-[55%] right-4 flex flex-col gap-1 items-start overflow-visible pointer-events-none z-10">
              {activePlayerBuffs.map((buff, i) => {
                ...
                return (
                  <div key={buff.skillId || i} className="flex items-center gap-1" title={buff.skillName}>
                    {folder ? (
                    ...
                    <span className="text-[9px] text-orange-300 whitespace-nowrap">{parts.join(" ")} {buff.turnsLeft}T</span>
                  </div>
                );
              })}
            </div>
