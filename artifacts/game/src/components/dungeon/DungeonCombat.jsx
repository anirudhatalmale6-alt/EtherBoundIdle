...
/* ── Player Card (matches game's existing UI style) ─────────────────────── */
function PlayerCard({ member, character, session, charSkills, isMyTurn, loading, doAction, onProfileClick, equippedPet }) {
  ...
  return (
    ...
    >
      <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${isDead ? "bg-muted/30" : "bg-primary/20"}`}>
        <img src={`/sprites/class_${member.class || "warrior"}.png`} alt={member.class} className="w-6 h-6" style={{ imageRendering: "pixelated", opacity: isDead ? 0.4 : 1 }} />
      </div>
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5 flex-wrap">
          {member.character_id === session.leader_id && <Crown className="w-3 h-3 text-accent flex-shrink-0" />}
          <span className={`text-xs font-medium ${CLASS_COLORS[member.class]} truncate`}>{member.name}</span>
          ...
          {isDead && <Badge variant="destructive" className="text-xs h-4 px-1">KO</Badge>}
        </div>
        <PixelBar current={Math.max(0, member.hp)} max={member.max_hp} type="hp" />
        {isMe && equippedPet && (
          <div className="mt-1 flex items-center gap-1.5 bg-muted/20 border border-border/50 rounded-lg px-2 py-1">
            ...
            <p className={`text-[9px] font-semibold leading-none truncate ${RARITY_COLORS[equippedPet.rarity] || "text-gray-400"}`}>
        ...
        <div className="bg-card border-2 border-destructive/30 rounded-xl p-4">
          <div className="flex items-center gap-3 mb-3">
            <div className="w-14 h-14 rounded-xl bg-destructive/20 border border-destructive/30 flex items-center justify-center">
              {session.boss_sprite ? (
                <img src={session.boss_sprite} alt={session.boss_name} className="w-10 h-10" style={{ imageRendering: "pixelated" }} />
              ) : (
          ...
          <div className="bg-card border border-border rounded-xl p-3 flex-1 min-h-0">
            ...
            <div ref={logRef} className="space-y-0.5 max-h-64 md:max-h-full overflow-y-auto">
              ...
              {(!session.combat_log || session.combat_log.length === 0) && (
                <p className="text-xs text-muted-foreground">Waiting for combat to begin...</p>
              )}
            </div>
          </div>
        </div>
