import React, { useState, useEffect } from "react";
import { motion } from "framer-motion";

export default function GemLabProgressBar({ gemLab, metrics }) {
  const [progressPct, setProgressPct] = useState(0);

  // Calculate progress based on time elapsed since last collection
  useEffect(() => {
    if (!gemLab?.last_collection_time || !metrics?.cycleTime) {
      setProgressPct(0);
      return;
    }

    const updateProgress = () => {
      const lastCollection = new Date(gemLab.last_collection_time).getTime();
      const now = Date.now();
      const elapsedSeconds = (now - lastCollection) / 1000;
      const cycleSeconds = metrics.cycleTime * 60; // Convert minutes to seconds
      
      // Progress = elapsed / cycle duration, capped at 100%
      const pct = Math.min((elapsedSeconds / cycleSeconds) * 100, 100);
      setProgressPct(pct);
    };

    updateProgress();
    
    // Update progress every 100ms for smooth animation
    const interval = setInterval(updateProgress, 100);
    return () => clearInterval(interval);
  }, [gemLab?.last_collection_time, metrics?.cycleTime]);

  return (
    <motion.div className="bg-card border border-border rounded-lg p-3 space-y-2">
      <div className="flex items-center justify-between text-xs">
        <span className="text-muted-foreground">Current Cycle</span>
        <span className="text-primary font-mono">{(metrics?.production ?? 0).toFixed(4)} gems</span>
      </div>
      <div className="w-full h-2 bg-muted/30 rounded-full overflow-hidden border border-border/50">
        <motion.div
          className="h-full bg-gradient-to-r from-primary via-secondary to-accent"
          animate={{ width: `${progressPct}%` }}
          transition={{ duration: 0.05 }}
        />
      </div>
      <p className="text-xs text-muted-foreground text-right">
        {progressPct.toFixed(0)}% — Next gems in ~{Math.max(0, ((100 - progressPct) / 100) * (metrics?.cycleTime || 0)).toFixed(1)}m
      </p>
    </motion.div>
  );
}